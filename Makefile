...
ARTIFACTBUNDLE_INFO_FILE = artifactbundle_info.md
...
RELEASE_ID = $(shell cat $(RESPONSE_PATH) \
	| python3 -c "import sys, json; print(json.load(sys.stdin)['id'])")

AUTOGENERATED_RELEASE_NOTE = $(shell cat $(RESPONSE_PATH) \
	| python3 -c "import sys, json; print(json.load(sys.stdin)['body'])" \
	| sed "s/\'/\\\u0027/g" | sed -e 's/$$/\\n/g' | tr -d '\r\n')

EXECUTABLE_UPLOAD_URL = $(shell cat $(RESPONSE_PATH) \
	| python3 -c "import sys, json; print(json.load(sys.stdin)['upload_url'])" \
	| sed -E 's/{.*}/?name=$(EXECUTABLE_ZIP)/')

ARTIFACTBUNDLE_UPLOAD_URL = $(shell cat $(RESPONSE_PATH) \
	| python3 -c "import sys, json; print(json.load(sys.stdin)['upload_url'])" \
...
define RELEASE_NOTE_TEMPLATE
$(AUTOGENERATED_RELEASE_NOTE)
$(ARTIFACTBUNDLE_INFO_TEMPLATE)
endef

# Replace " with \" after replace newline with \n
RELEASE_NOTE = $(subst ",\",$(subst $(NEWLINE),\n,$(RELEASE_NOTE_TEMPLATE)))
...
.PHONY: zip
zip: zip-executable zip-artifactbundle generate-artifact-info
...
.PHONY: generate-artifact-info
generate-artifact-info:
	echo '$(ARTIFACTBUNDLE_INFO)' > $(ARTIFACTBUNDLE_INFO_FILE)
...
.PHONY: distribute
distribute: archive zip create-release upload update-release-note open-release-page
...
.PHONY: update-release-note
update-release-note:
	@echo Update a release note

	curl -X PATCH \
		-H "Authorization: token $(GITHUB_TOKEN)" \
		-H "Accept: application/vnd.github.v3+json" \
		-H "Content-Type:application/json" \
		-d '{"tag_name":"$(RELEASE_TAG)","target_commitish":"main","body":"$(RELEASE_NOTE)"}' \
		-o "$(RESPONSE_PATH)" \
